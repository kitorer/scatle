...
const HelpModal = ({ setHelpOff }) => {
    ...
    return (
        <div className="fixed z-10 inset-0 overflow-y-auto" onClick={ ()=> { setHelpOff(false)}} >
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity">
            <span className="hidden sm:inline-block sm:align-middle sm:h-screen " >
                <div className=" inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6 h-fit" onClick={ (e)=> {e.stopPropagation}}>
